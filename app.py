...
from src.logic import (
    ...
    run_llm_simulation, get_recipes, read_recipe
)
import os
...
class MinerManagerWindow(ctk.CTkToplevel):
    ...
    def on_miner_selected(self, selection):
        if selection == "Add New Miner":
            ...
            miner_data = next((m for m in self.config['miners'] if m['name'] == selection), None)
            if miner_data:
    ...
    def save_miner(self):
        ...
        else: new_data['device'] = None
        if selected_name == "Add New Miner": self.config['miners'].append(new_data)
        else:
            for i, miner in enumerate(self.config['miners']):
                if miner['name'] == selected_name: self.config['miners'][i] = new_data; break
    ...
    def delete_miner(self):
        ...
        if selected_name != "Add New Miner":
            self.config['miners'] = [m for m in self.config['miners'] if m['name'] != selected_name]
            write_config(self.config)
            self.refresh_selectors()
...
class LocalLLMApp(ctk.CTk):
    ...
    def create_ai_tab(self, tab):
        ...
        recipe_frame.grid(row=0, column=1, padx=(5, 10), pady=10, sticky="nsew")
    ...
    def run_llm(self):
        ...
        response = run_llm_simulation(prompt)
    ...
    def load_recipe(self, selection):
        ...
        recipe_data = read_recipe(category, name)
        if recipe_data:
    ...
    def refresh_miner_list(self):
        ...
        self.miner_widgets = {}
        config = read_config()
        for i, miner_config in enumerate(config.get('miners', [])):
            ...
            start_btn = ctk.CTkButton(frame, text="Start", width=60, command=lambda m=miner_config: start_miner(m)); start_btn.grid(row=0, column=2, padx=5, pady=5)
            stop_btn = ctk.CTkButton(frame, text="Stop", width=60, command=lambda n=miner_name: stop_miner(n)); stop_btn.grid(row=0, column=3, padx=5, pady=5)
    ...
    def start_all_miners(self):
        for miner_config in read_config().get('miners', []):
            if miner_config['name'] not in RUNNING_MINERS: start_miner(miner_config)

    def stop_all_miners(self):
        for miner_name in list(RUNNING_MINERS.keys()): stop_miner(miner_name)
    ...
    def update_dashboard(self):
        ...
        self.cpu_progress.set(stats['cpu']['percent'] / 100); self.cpu_label.configure(text=f"{stats['cpu']['percent']:.1f}%")
        ...
        if stats['gpus'] and not self.gpu_frames:
            for i, gpu_stat in enumerate(stats['gpus']):
                ...
                ctk.CTkLabel(gpu_frame, text=f"GPU {i}: {gpu_stat['name']}").grid(row=0, column=0, columnspan=3, padx=10, pady=5, sticky="w")
                ...
                gpu_mem_label = ctk.CTkLabel(gpu_frame, text="0%"); gpu_mem_label.grid(row=2, column=2, padx=1.0, pady=2, sticky="e")
        ...
        for i, gpu_stat in enumerate(stats['gpus']):
            if i < len(self.gpu_frames):
                ...
                frame_info['usage_progress'].set(gpu_stat['usage'] / 100); frame_info['usage_label'].configure(text=f"{gpu_stat['usage']}%")
                frame_info['mem_progress'].set(gpu_stat['memory_usage'] / 100); frame_info['mem_label'].configure(text=f"{gpu_stat['memory_usage']}%")
                frame_info['temp_label'].configure(text=f"{gpu_stat['temperature']}°C")
